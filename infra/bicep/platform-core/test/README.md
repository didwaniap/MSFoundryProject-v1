# Test Platform Core Bicep

This resource-group-scope Bicep module defines the shared Test platform services for the Microsoft Foundry demo.
...
```text
Environments > What-if Test Core
```
...
az deployment group what-if \
  --resource-group msfoundryv1-platform-test-rg \
  --name msfoundryv1-platform-core-test \
  --template-file infra/bicep/platform-core/test/main.bicep \
  --parameters infra/bicep/platform-core/test/main.parameters.json
```
...
```text
Teardown > Teardown Test Core
```

This deletes Test Core resources inside `msfoundryv1-platform-test-rg` but preserves the resource group and the foundation layout.
